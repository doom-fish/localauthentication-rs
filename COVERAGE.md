...
| `LAContext.h` | `evaluateAccessControl:operation:localizedReason:reply:` | ✅ | Exposed as the safe `LAContext::evaluate_access_control` and `AsyncContextExt::evaluate_access_control_async`, which take `security-rs`'s `AccessControl` (a real `SecAccessControlRef`, re-exported here). `LAContext::as_raw_la_context()` returns the Objective-C context for `kSecUseAuthenticationContext` keychain queries (`security-rs`'s `KeychainOptions::authentication_context`). |
